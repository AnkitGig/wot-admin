...
export default function Glossaries() {
  ...
  const fetchGlossaries = async (pageNumber = 1) => {
    ...
    if (!token) {
      const { data, total, count } = applyDummyFilter(
        DUMMY_GLOSSARIES,
        "",
        pageNumber,
        pagination.limit,
    ...
    try {
      const result = await getAllGlossaries(
        token,
        pageNumber,
        pagination.limit,
        {
  ...
  const handleSearch = async (pageNumber = 1) => {
    ...
    setError(null);
    try {
      const result = await searchGlossaries(
        token,
        searchTerm.trim(),
        pageNumber,
        pagination.limit,
  ...
  const handleEditClick = (glossary) => {
    ...
    setEditFormData({
      ...
      category_en: hasNested
        ? glossary.category?.en || ""
        : glossary.category || "",
      category_fr: hasNested
        ? glossary.category?.fr || ""
        : glossary.category || "",
      category_es: hasNested
        ? glossary.category?.es || ""
        : glossary.category || "",
      description_en: hasNested
        ? glossary.description?.en || ""
        : glossary.description || "",
      description_fr: hasNested
        ? glossary.description?.fr || ""
        : glossary.description || "",
      description_es: hasNested
        ? glossary.description?.es || ""
        : glossary.description || "",
    });
  ...
  const handleEditSubmit = async (e) => {
    ...
    if (!editFormData.category_en.trim()) {
      Swal.fire({
        icon: "warning",
        title: "Validation Error",
        text: "English category is required.",
      });
      return;
    }
    ...
    setIsLoading(true);
    try {
      const updateResult = await updateGlossary(
        editingGlossary.id,
        payload,
        token,
      );
  ...
  const handleDeleteGlossary = (glossaryId, glossaryTerm) => {
    if (isUsingDummyData || !token) {
      Swal.fire({
        icon: "info",
        title: "Demo Mode",
        text: "Delete is disabled because the API is unavailable.",
      });
      return;
    }
  ...
  const handleBulkDelete = () => {
    ...
    if (isUsingDummyData || !token) {
      Swal.fire({
        icon: "info",
        title: "Demo Mode",
        text: "Bulk delete is not available in demo mode.",
      });
      return;
    }
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        setIsLoading(true);
        try {
          const deleteResult = await deleteAllGlossaries(
            selectedGlossaries,
            token,
          );
          if (deleteResult.success) {
  ...
  const getDisplayTerm = (glossary) => {
    if (glossary.term && typeof glossary.term === "object")
      return glossary.term.en || "";
    return glossary.term || "";
  };

  const getDisplayCategory = (glossary) => {
    if (glossary.category && typeof glossary.category === "object")
      return glossary.category.en || "";
    return glossary.category || "";
  };

  const getDisplayDescription = (glossary) => {
    if (glossary.description && typeof glossary.description === "object")
      return glossary.description.en || "";
                ...
                <ul className="filter-list">
                  ...
                  <li>
                    <Link
                      ...
                      to="/glossary-category/add"
                    >
                      <i className="fa fa-plus-circle me-2"></i>Add Glossary
                      Category
                    </Link>
                  </li>
                  <li>
                          ...
                          <th style={{ width: "50px" }}>
                            <div className="form-check">
                              <input
                                ...
                                disabled={
                                  glossaries.length === 0 || isUsingDummyData
                                }
                              />
                            </div>
                          </th>
                        ...
                        {isLoading ? (
                          ...
                          glossaries.map((glossary) => (
                            <tr key={glossary.id}>
                              <td>
                                <div className="form-check">
                                  <input
                                    type="checkbox"
                                    className="form-check-input"
                                    checked={selectedGlossaries.includes(
                                      glossary.id,
                                    )}
                                    onChange={() =>
                                      handleSelectOne(glossary.id)
                                    }
                                    disabled={isUsingDummyData}
                                  />
                                </div>
                              </td>
                              <td>
                              ...
                              <td>
                                <small title={getDisplayDescription(glossary)}>
                                  {getDisplayDescription(glossary)?.substring(
                                    0,
                                    60,
                                  )}
                                  ...
                ...
                <div className="card">
                  <div className="card-body d-flex justify-content-between align-items-center">
                    <small className="text-muted">
                      Page {pagination.page} of {totalPages} | Showing{" "}
                      {glossaries.length} of {pagination.total} glossaries
                    </small>
                    <nav>
      ...
      >
        <div className="modal-dialog modal-dialog-centered modal-xl">
          <div className="modal-content border-0 shadow-lg">
            ...
            >
              <h5 className="modal-title text-light">Edit Glossary</h5>
              ...
              <div className="modal-body">
                ...
                >
                  ...
                  <div className="card-body">
                    ...
                    {/* CATEGORY */}
                    <div className="mb-3">
                      <label className="form-label">English Category</label>
